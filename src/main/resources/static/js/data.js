...
function renderNoticeStylePanel(isPromoLayout) {
  const pagination = document.querySelector(".pagination");
  const dashboardContent = pagination?.closest(".dashboard-content");
  const actionAreaNode = document.querySelector(".pagination .action-area");

  if (!pagination || !dashboardContent || !actionAreaNode) {
    return;
  }
  ...
  if (!isPromoLayout) {
    ...
    if (existingPanel) {
      if (existingPanel.contains(actionAreaNode)) {
        pagination.appendChild(actionAreaNode);
      }
  ...
  searchWrap.innerHTML = `
    <select class="notice-search-select" name="searchopt" aria-label="홍보자료 검색 조건">
    ...
    <input
      ...
      placeholder="제목으로 검색"
      aria-label="홍보자료 제목 검색"
    />
    <button type="button" class="notice-search-btn">검색</button>
  `;

  controlsPanel.appendChild(actionAreaNode);

  actionAreaNode.innerHTML = "";
  actionAreaNode.appendChild(createNoticeActionButton("순서변경"));
  ...
  actionAreaNode.appendChild(rightArea);
}

function renderDashboardTable() {
  const config = getDashboardTableConfig();
  const isPromoLayout = state.activeTab === "posts" && state.activeSubMenu === "promo";
  const tableWrap = tableBody.closest(".table-wrap");
  const rows = config.rows || [];
  renderNoticeStylePanel(isPromoLayout);
  ...
  if (isPromoLayout) {
    ...
    if (tableHead) {
      tableHead.parentElement.closest("table").style.display = "none";
    }
    ...
    if (promoGrid) {
      ...
      const gridRows = [];
      for (let i = 0; i < visibleRows.length; i += 4) {
        const rowItems = visibleRows.slice(i, i + 4).map((row) => {
          const title = row[1] || "";
  ...
  if (tableHead) {
    tableHead.parentElement.closest("table").style.display = "";
  }
  ...
  if (actionArea) {
    const defaultActionButton = tableActionBtn && tableActionBtn.tagName === "BUTTON" ? tableActionBtn : document.createElement("button");
    defaultActionButton.type = "button";
    defaultActionButton.className = "add-btn";
    defaultActionButton.innerHTML = `<span class="icon"><img src="/asset/img/plus.gif" alt="" /></span>${config.actionLabel}`;
    actionArea.innerHTML = "";
    actionArea.appendChild(defaultActionButton);
  } else if (tableActionBtn) {
    tableActionBtn.classList.remove("promo-action-btn");
    tableActionBtn.classList.add("add-btn");
