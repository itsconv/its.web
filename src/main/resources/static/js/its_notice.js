...
const dashboardTableMap = {
  ...
  notice: {
    ...
    rows: [
      ...
      ["2", "[공지] ITS 신입/경력사원 채용안내", "관리자"],
      ["1", "[공지] 홈페이지 신규 오픈 안내", "관리자"]
    ],
...
const paginationController = AdminLayout.createPaginationController(state, {
  ...
  pageRight1,
  pageRight2,
  isEnabled: () => state.activeTab === "posts" && state.activeSubMenu === "notice",
  getTotalPages: () => {
...
function getDashboardTableConfig() {
  ...
  if (state.activeTab === "posts" && state.activeSubMenu === "notice") {
    return dashboardTableMap.notice;
  }
...
function renderActionButtons(isNotice) {
  if (!tableActionBtn) {
    return;
  }
  ...
  if (pagination) {
    const controlsPanel = pagination.closest(".dashboard-content")?.querySelector(
      ".notice-controls-panel"
    );
    if (controlsPanel) {
  ...
  tableActionBtn.innerHTML = `<span class="icon"><img src="/asset/img/plus.gif" alt="" /></span>${dashboardTableMap[isNotice ? "notice" : state.activeTab === "basic" ? "admin" : "popup"].actionLabel}`;
}

function renderDashboardTable() {
  const config = getDashboardTableConfig();
  const isNotice = state.activeTab === "posts" && state.activeSubMenu === "notice";
